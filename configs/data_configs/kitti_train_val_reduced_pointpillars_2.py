...
# Below Must be set
#################################################################
model_name = "hv_pointpillars_secfpn_6x8_160e_kitti-3d-3class"
split = "_train_val"
reduced_velodyne = True
velodyne_suffix = "_reduced" if reduced_velodyne else ""

model_cfg_path = osp.join(pc.MMDET3D_OUT_DIR, f"pointpillars/{model_name}{split}{velodyne_suffix}/{model_name}{split}{velodyne_suffix}.py")
model_ckpt_path = osp.join(pc.MMDET3D_OUT_DIR, f"pointpillars/{model_name}{split}{velodyne_suffix}/epoch_78.pth")
...
if split == "_train_val_test":
    ...
    print(f"Split {self.split} is not correct, must be either '_train_val_test' or '_train_val'.")
