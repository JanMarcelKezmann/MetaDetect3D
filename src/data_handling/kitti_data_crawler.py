...
class KITTIDataCrawler(DataHandler):
    ...
    def get_pred_labels(self, filename):
        ...
        output, proposals, data = kitti_inference_detector(self.model, osp.join(self.train_velo_dir, filename))
            ...
            if "centerpoint" in str(self.model.__class__).lower():
                row = [
                    ...
                    *box_3d.cpu().numpy(),                          # x_center, y_center, z_center, length, width, height, theta
                    np.amax(scores),                                # max_score
                    np.argmax(scores),                              # category_idx
                    pib_num,                                        # num_pib
